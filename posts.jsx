        ...
        <div className="col-2">
          <div>
            ...
            <div className="col-2b">
              <small>Sample Date: {post.startdate}</small>
            </div>
            <div className="col-2b">
              <div>Water Temperature:</div>
              <div>
                {post.temperature}
                <sup>º</sup>F ~ {Math.round(wt.toFixed(2))}
                <sup>º</sup>C
              </div>
            </div>
            <div className="col-2b"></div>
            <div className="col-2b">
              <div>Air Temperature:</div>
              <div>
                {post.AirTemp}
                <sup>º</sup>F ~ {Math.round(t.toFixed(2))}
                <sup>º</sup>C
              </div>
            </div>
            <>
            ...
            <div>{post.algae && <div> Algae: {post.algae}</div>}</div>
            <div>
              <p>
          ...
          <div className="center">
            <div>
              <div>
                {post.temperature <= "59" ? (
                  <span className="cool">{post.temperature} </span>
                ) : null}
                {post.temperature >= "60" && post.temperature <= "69" ? (
                  <span className="warn">{post.temperature} </span>
                ) : null}
                {post.temperature >= "70" ? (
                  <span className="hot">{post.temperature} </span>
                ) : null}
              </div>
            </div>
          </div>
